...
def findwcs(filehandle, rashift = -0.0, decshift = -0.0, match_limit = 1*u.arcsec):
    ...
    tables = Vizier(row_limit=10000).query_region(center, radius=4.5*u.arcmin, catalog='gaia')
    table = tables[2]
    table_coordinates = coordinates.SkyCoord(table['RA_ICRS'], table['DE_ICRS'], frame=None)
    brightstars = table['RPmag'] < 17
    
    ww = WCS(filehandle[0].header)
    ww.wcs.crval[1] += decshift/3600
    ww.wcs.crval[0] += rashift/3600
    
    std = mad_std(filehandle[0].data)
    daofind = DAOStarFinder(fwhm=3.0, threshold=30.*std)
    ...
    bad_matches = (ra_residual < -match_limit) | (ra_residual > match_limit) | (dec_residual > match_limit) | (dec_residual < -match_limit)
    
    xy_starfind2 = xy_starfind.T[~bad_matches].T
    new_sky_coordinates2 = new_sky_coordinates.T[~bad_matches].T
    new_wcs2 = fit_wcs_from_points(xy_starfind2, new_sky_coordinates2, sip_degree=4)
